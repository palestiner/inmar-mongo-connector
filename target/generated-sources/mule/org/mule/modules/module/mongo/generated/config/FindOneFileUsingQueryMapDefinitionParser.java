...
import org.mule.modules.module.mongo.generated.processors.FindOneFileUsingQueryMapMessageProcessor;
import org.mule.modules.module.mongo.generated.processors.FindOneFileUsingQueryMapMessageProcessorDebuggable;
    ...
    extends AbstractDevkitBasedDefinitionParser
{
    ...
    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            ...
            if (muleVersion.atLeastBase("3.8.0")) {
                beanDefinitionBuilder = beanDefinitionBuilder = BeanDefinitionBuilder.rootBeanDefinition(FindOneFileUsingQueryMapMessageProcessorDebuggable.class.getName());
            } else {
                beanDefinitionBuilder = beanDefinitionBuilder = BeanDefinitionBuilder.rootBeanDefinition(FindOneFileUsingQueryMapMessageProcessor.class.getName());
            }
    ...
    public BeanDefinition parse(Element element, ParserContext parserContext) {
        ...
        if (!hasAttribute(element, "config-ref")) {
            throw new BeanDefinitionParsingException(new Problem("It seems that the config-ref for @Processor [find-one-file-using-query-map] within the connector [mongo] is null or missing. Please, fill the value with the correct global element.", new Location(parserContext.getReaderContext().getResource()), null));
        }
