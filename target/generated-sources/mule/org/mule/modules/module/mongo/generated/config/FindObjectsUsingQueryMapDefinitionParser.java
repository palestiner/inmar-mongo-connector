...
import org.mule.modules.module.mongo.generated.processors.FindObjectsUsingQueryMapMessageProcessor;
import org.mule.modules.module.mongo.generated.processors.FindObjectsUsingQueryMapMessageProcessorDebuggable;
    ...
    extends AbstractDevkitBasedDefinitionParser
{
    ...
    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            ...
            if (muleVersion.atLeastBase("3.8.0")) {
                beanDefinitionBuilder = beanDefinitionBuilder = BeanDefinitionBuilder.rootBeanDefinition(FindObjectsUsingQueryMapMessageProcessorDebuggable.class.getName());
            } else {
                beanDefinitionBuilder = beanDefinitionBuilder = BeanDefinitionBuilder.rootBeanDefinition(FindObjectsUsingQueryMapMessageProcessor.class.getName());
            }
    ...
    public BeanDefinition parse(Element element, ParserContext parserContext) {
        ...
        if (!hasAttribute(element, "config-ref")) {
            throw new BeanDefinitionParsingException(new Problem("It seems that the config-ref for @Processor [find-objects-using-query-map] within the connector [mongo] is null or missing. Please, fill the value with the correct global element.", new Location(parserContext.getReaderContext().getResource()), null));
        }
        ...
        if (hasAttribute(element, "sortBy-ref")) {
            if (element.getAttribute("sortBy-ref").startsWith("#")) {
                builder.addPropertyValue("sortBy", element.getAttribute("sortBy-ref"));
            } else {
                builder.addPropertyValue("sortBy", (("#[registry:"+ element.getAttribute("sortBy-ref"))+"]"));
            }
        }
