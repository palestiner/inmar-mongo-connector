...
import org.mule.modules.module.mongo.generated.processors.FindOneObjectUsingQueryMapMessageProcessor;
import org.mule.modules.module.mongo.generated.processors.FindOneObjectUsingQueryMapMessageProcessorDebuggable;
    ...
    extends AbstractDevkitBasedDefinitionParser
{
    ...
    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            ...
            if (muleVersion.atLeastBase("3.8.0")) {
                beanDefinitionBuilder = beanDefinitionBuilder = BeanDefinitionBuilder.rootBeanDefinition(FindOneObjectUsingQueryMapMessageProcessorDebuggable.class.getName());
            } else {
                beanDefinitionBuilder = beanDefinitionBuilder = BeanDefinitionBuilder.rootBeanDefinition(FindOneObjectUsingQueryMapMessageProcessor.class.getName());
            }
    ...
    public BeanDefinition parse(Element element, ParserContext parserContext) {
        ...
        if (!hasAttribute(element, "config-ref")) {
            throw new BeanDefinitionParsingException(new Problem("It seems that the config-ref for @Processor [find-one-object-using-query-map] within the connector [mongo] is null or missing. Please, fill the value with the correct global element.", new Location(parserContext.getReaderContext().getResource()), null));
        }
